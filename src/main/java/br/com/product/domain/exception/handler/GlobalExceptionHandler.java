...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleUncaught(Exception ex, WebRequest request) {
        ...
        Error problem = problem(status, problemType, detail);
    ...
    private Error problem(HttpStatus status, ErrorType problemType, String detail) {
        return problem(status, problemType, detail, null);
    }

    private Error problem(HttpStatus status, ErrorType problemType, String detail, String messageError) {

        Error erro = null;

        if (!Strings.isEmpty(messageError)) {
            erro = new Error(status.value(), LocalDateTime.now(), detail, null);
        } else {
            erro = new Error(status.value(), LocalDateTime.now(), detail, messageError, null);
        }
